...
CARD_DB = {
    ...
    "慈悲 of 祝福": Card("慈悲 of 祝福", "heal", 35, check_pair, "ペア", "HPを回復。"),
}
INNATE_DECK = [
...
def update_db(u): 
    ...
    except Exception: pass # カラムがない場合などのエラー回避
...
<style>
    ...
    div.stButton > button[key^="atk_"] {
        background-color: #FFA500 !important; color: black !important; font-weight: bold !important; border-radius: 8px !important;
    }
    ...
    .opp-dice-box { border-color: #FF4B4B; color: #FF4B4B; opacity: 0.7; font-size: 25px; padding: 10px; }
</style>
...
with c2:
    ...
    d_cols = st.columns(5)
    for i in range(5):
        d_cols[i].markdown(f"<div class='dice-box opp-dice-box'>{o_dice[i]}</div>", unsafe_allow_html=True)
...
if data["turn"] == (f"P{my_id}"):
    ...
    for idx, card in enumerate(available):
        ...
        if st.button(f"【{card.cond_text}】{card.name} ({total_p} dmg/hp)", key=f"atk_{idx}"):
    ...
    with col_x:
        ...
        if st.session_state.rolls_left > 0:
            if st.button(f"🎲 選択以外を振り直す (残り{st.session_state.rolls_left})", key="reroll_btn"):
                for i in range(5):
    ...
    st.info("相手のターンです。リアルタイム同期中...")
    time.sleep(2)
    st.rerun()

if st.sidebar.button("🚨 全リセット(青)", key="reset_all"):
    update_db({"hp1": 100, "hp2": 100, "turn": "P1", "turn_count": 0, "p1_used_innate": [], "p2_used_innate": [], "p1_bonus": 0, "p2_bonus": 0, "p1_dice": [1,1,1,1,1], "p2_dice": [1,1,1,1,1]})
